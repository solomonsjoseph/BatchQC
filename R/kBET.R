...
kBET <- function(
    ...
    if (is.null(k0) || k0 >= dim.dataset[1]) {
        ...
        if (!heuristic) {
            ...
            # default environment size: three quarter the size of the largest
            # batch
            k0 <- floor(mean(class.frequency$freq) * dim.dataset[1] * 0.75)
            if (k0 < 10) {
                if (verbose) {
                    warning(
                        "Your dataset has too few samples to ",
                        "run a heuristic.\nReturn NA.\n",
    ...
    rejection$average.pval <- 1 - pchisq(
        k0 * residual_score_batch(env, cf, batch),
        dof
    )
    ...
    if (addTest) {
        ...
        for (i in seq_len(n_repeat)) {
            ...
            # (approximation for multinomial exact test)
        ...
        for (i in seq_len(n_repeat)) {
            ...
            p.val.test.null <- apply(
                ...
                function(x) {
                    apply(
                        env, 1, chi_batch_test,
                        class.frequency, x, dof
                    )
                }
            )
            ...
            rejection$results$tested[idx.runs] <- 1
            rejection$results$kBET.pvalue.test[idx.runs] <- p.val.test
            rejection$results$kBET.pvalue.null[idx.runs] <-
                rowMeans(p.val.test.null, na.rm = TRUE)
        }
